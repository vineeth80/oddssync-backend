...
Uses a persistent connection with WAL mode to avoid per-query connection overhead.
"""
...
async def _get_db() -> aiosqlite.Connection:
    """Return the shared DB connection, creating it if needed."""
    global _db
    if _db is None:
        ...
        await _db.execute("PRAGMA cache_size=-8000")  # 8 MB cache
    return _db
...
async def get_all_matched_markets() -> list[dict[str, Any]]:
    ...
    db = await _get_db()
    cursor = await db.execute(
        _MATCHED_QUERY + " ORDER BY mm.match_confidence DESC"
    )
    return [dict(r) for r in await cursor.fetchall()]


async def get_matched_markets_filtered(
    *,
    ...
    """Return matched markets with SQL-level filtering, sorting, pagination.
    ...
    if category:
        where_clauses.append("k.category LIKE ?")
        params.append(f"%{category}%")

    if search:
        where_clauses.append("(k.title LIKE ? OR p.question LIKE ?)")
    ...
    count_sql = f"""
        SELECT COUNT(*) FROM matched_markets mm
        JOIN kalshi_markets k ON mm.kalshi_ticker = k.ticker
        JOIN poly_markets p ON mm.poly_id = p.id
        {where_sql}
    """
    ...
    # Sort mapping — push sorting to SQL where possible
    dir_sql = "DESC" if order != "asc" else "ASC"
    sort_map = {
        "spread": f"ABS(k.yes_price - p.yes_price) {dir_sql}",
